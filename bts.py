...
tv_vs_pop_hist_rec.replace(np.nan,'0')
...
summ_df = summary_df.sort_values(sort_col,ascending=False).reset_index(drop=True).round(0).astype(str).replace('nan','0')
summ_df.style.format(thousands=',',na_rep='',precision=0)\
...
summ_df1 = summary_df1.sort_values(sort_col,ascending=False).reset_index(drop=True).astype(str).replace('nan','0')


l1 = "LICENSE AND NO LICENSE"
l2 = "OBSOLETE AND OTHERS"

s1 = len(summ_df) - 1
s2 = len(summ_df1) - 1

LIC = tv_vs_pop_obsolet['License_Required'].value_counts()['LIC REQD (OBSOLETE)']




st.title(f":bar_chart: BTS INVENTORY COUNT {l1}")
st.write(summ_df)
st.write(f'Total :  {s1}')
st.download_button(label='Donwload CSV',data=summ_df.to_csv(),file_name=f' {l1} {now}.csv')
st.pyplot(fig)
st.title(f":bar_chart: BTS INVENTORY COUNT {l2}")
st.dataframe(summ_df1) 
st.write(f'Total :  {s2}')
st.write(LIC)
st.download_button(label='Donwload CSV',data=summ_df1.to_csv(),file_name=f' {l2} {now}.csv')
st.pyplot(fig2)
